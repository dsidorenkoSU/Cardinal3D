...
Ray Camera::generate_ray(Vec2 screen_coord) const {
    ...
    float horz_len = tan(horz_fov * M_PI / 180.0f / 2.0f) * 2.0f; 
    ...
    Mat4 iview_i = iview;
    Ray ray_world(iview_i.operator*(origin), iview_i.operator*(origin.operator-(pixel_coord))); // ray in world space 
    // Ray ray_world(iview_i.operator*(origin), iview_i.operator*(Vec3(0,0,-1))); // ray in world space 
